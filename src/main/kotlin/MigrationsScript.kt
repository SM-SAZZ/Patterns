...
import org.sazz.db.migrationstore.JsonMigrationStore
import org.sazz.db.migrationstore.MigrationStoreInterface
import java.io.File
    ...
    if (migrationsDir.exists() && migrationsDir.isDirectory) {
        migrationsDir.walk().filter { it.isFile && it.name.endsWith(".kt") }.forEach { file ->
            ...
            if (!excludedClasses.contains(className)) {
                className = "org.sazz.migrations.$className"
...
fun initInstancesForDown(count: Int): List<Migration> {
    ...
    return migrationList.asSequence()
        .map { migrationName ->
            val className = "org.sazz.migrations." + migrationName
            val kClass = Class.forName("$className").kotlin
